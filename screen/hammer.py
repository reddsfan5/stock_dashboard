...
def find_all(data: StockData, **kwargs) -> pd.DataFrame:
    ...
    results = []
    for code, group in tqdm(recent.groupby("代码"), desc="金针探底", **tqdm_kwargs):
        group = group.sort_values("日期").tail(LOOKBACK)
        if len(group) < LOOKBACK:
            continue

        # 内联 analyze_hammer 的核心逻辑
        closes = group["收盘"].values
        has_open = "开盘" in group.columns
        if has_open:
            opens = group["开盘"].values
        else:
            opens = np.concatenate([[closes[0]], closes[:-1]])
        highs = group["最高"].values
        lows = group["最低"].values
        amounts = group["成交额"].values

        best = None
        for i in range(max(0, len(group) - HAMMER_DAYS), len(group)):
            o = opens[i] if opens[i] > 0 else closes[i - 1] if i > 0 else closes[i]
            c = closes[i]
            h = highs[i]
            l = lows[i]
            ...
            score = (
                min(shadow_ratio / 5, 1) * 35
                + (1 - bottom_pos / 100) * 25
                + (1 if is_yang else 0) * 15
                + min(vol_ratio / 2, 1) * 10
                + (1 if follow_up > 0 else 0) * 15
            )
