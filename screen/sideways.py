...
def find_all(data: StockData, **kwargs) -> pd.DataFrame:
    ...
    MAX_AMPLITUDE = kwargs.pop("max_amp", MAX_AMPLITUDE)
    MIN_AMOUNT = kwargs.pop("min_amount", MIN_AMOUNT)
    MAX_SLOPE = kwargs.pop("max_slope", MAX_SLOPE)
    MIN_OVERLAP = kwargs.pop("min_overlap", MIN_OVERLAP)
    MIN_R_SQUARED = kwargs.pop("r2_max", MIN_R_SQUARED)
    days = kwargs.pop("days", DAYS)
    tqdm_kwargs = kwargs.pop("_tqdm_kwargs", {})
    return find_sideways(data, days=days, _tqdm_kwargs=tqdm_kwargs)


def find_sideways(data: StockData, days: int = DAYS, _tqdm_kwargs: dict = None) -> pd.DataFrame:
    if _tqdm_kwargs is None:
        _tqdm_kwargs = {}
    """
    找出横盘震荡的股票
    ...
    recent_dates = all_dates[-max(days * 2, 20):]
    cache = data.cache
    mask = cache["日期"].isin(recent_dates) & cache["代码"].isin(main_codes)
    recent = cache[mask]
    name_map = {c: data.get_stock_name(c) for c in main_codes}

    results = []
    for code, group in tqdm(recent.groupby("代码"), desc="横盘震荡", **_tqdm_kwargs):
        group = group.sort_values("日期").tail(days)
        if len(group) < days:
            continue

        metrics = calc_consolidation_score(group)
