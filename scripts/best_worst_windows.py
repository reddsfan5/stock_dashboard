...
用法
----
$ python scripts/best_worst_windows.py                     # 上证指数 30 交易日，最好/最差各 3 段
$ python scripts/best_worst_windows.py --window 60         # 60 个交易日
$ python scripts/best_worst_windows.py --top 5             # 各 5 段
$ python scripts/best_worst_windows.py --index sh000300    # 沪深300
...
必须用 /usr/local/bin/python 3.9.9（项目全部依赖所在，定时任务同款）：
    /usr/local/bin/python scripts/best_worst_windows.py
...
脚本: scripts/best_worst_windows.py · 段间不重叠（贪心剔除重叠窗口）· 数据: 本地指数缓存</div>
</body></html>"""
