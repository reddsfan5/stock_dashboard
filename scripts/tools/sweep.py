...
用法
----
$ python -m scripts.tools.sweep --universe etf                       # ETF 池（默认，秒级）
$ python -m scripts.tools.sweep --universe etf --fast 5,10,20 --slow 20,30,60
$ python -m scripts.tools.sweep --universe stock                     # 主板股票（吃指标缓存）
$ python -m scripts.tools.sweep --universe etf --top 20 --heatmap    # 输出热力图
...
py3.9 下 vectorbt 锁 numba<0.57→numpy<1.24），其余脚本用 /usr/local/bin/python：
    .venv/bin/python -m scripts.tools.sweep --universe etf
