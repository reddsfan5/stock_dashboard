...
.sub{font-size:12px;color:var(--muted);margin-top:4px;line-height:1.45;max-width:560px}
.search{display:flex;gap:8px;align-items:center;margin-top:10px;min-width:min(560px,100%);flex-wrap:wrap}
.search input{flex:1;min-width:140px;background:#0b1220;border:1px solid var(--line);color:var(--text);border-radius:10px;padding:10px 12px;font-size:14px;outline:none}
...
.side{top:16px;right:16px;width:min(360px,92vw);max-height:calc(100vh - 32px)}
.panel{max-height:calc(100vh - 32px);overflow:auto}
...
.board{position:fixed;left:16px;top:230px;width:min(280px,40vw);z-index:5;max-height:calc(100vh - 300px);display:flex;flex-direction:column}
.board .hd{font-size:13px;font-weight:600;margin-bottom:6px;display:flex;justify-content:space-between}
.board .scroll{overflow:auto;flex:1;min-height:120px}
.members{max-height:220px;overflow:auto}
.foot{position:fixed;bottom:14px;left:16px;font-size:11px;color:var(--muted);background:var(--panel);border:1px solid var(--line);border-radius:10px;padding:8px 10px;max-width:min(780px,90vw);z-index:4}
.empty{color:var(--muted);font-size:12px;line-height:1.5}
@media(max-width:900px){.board{display:none}.side{top:auto;bottom:12px;left:12px;right:12px;width:auto;max-height:36vh}}
</style>
</head>
<body>
<canvas id="c"></canvas>
...
<aside class="board card">
  <div class="hd"><span>涨跌榜</span><span class="empty" id="boardLabel">当日</span></div>
  <ul class="list scroll" id="boardList"></ul>
</aside>
...
const pointsMat = new THREE.PointsMaterial({ size: 3.6, vertexColors: true, transparent: true, opacity: 0.95, sizeAttenuation: true, depthWrite: false });
...
function retOf(n) {
  const v = retWindow === 'cum20' ? n.cum20 : n.last;
...
function sizeFromRet(v) {
  ...
  return 2.6 + 5.2 * Math.min(1, Math.abs(v) / retScale());
}
