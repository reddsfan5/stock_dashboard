...
用法
----
$ python -m scripts.reports.quant_report --equity output/etf_momentum_equity.csv
$ python -m scripts.reports.quant_report --equity output/etf_momentum_equity.csv \\
    --trades output/etf_momentum_trades.csv --benchmark sh510300
$ python -m scripts.reports.quant_report --equity ... --out output/quant_report.html

CSV 来源: scripts/strategies/strategy_08_etf_momentum.py 已自动导出
...
def main():
    ...
    if not os.path.exists(args.equity):
        sys.exit(f"✗ 权益 CSV 不存在: {args.equity}\n"
                 f"  先跑 python -m scripts.strategies.strategy_08_etf_momentum 生成")
