...
用法
----
$ python scripts/gen_market.py           # 生成桌面+手机页面并刷新导航
$ python scripts/gen_market.py --no-nav  # 不刷新导航
"""
...
<script>{js}</script>
<div class="footer">生成: scripts/gen_market.py · 数据: 个股K线缓存 + 腾讯指数日K（每日 18:30 自动更新）<br>
...
<script>window.echarts || document.write(`<script src='https://cdn.jsdelivr.net/npm/echarts@5.4.3/dist/echarts.min.js'><\\/script>`);</script>
<script>{js}</script>
<div class="footer">scripts/gen_market.py · 涨跌停按 主板10%/创业板20%/ST≈5% 估算 · 申万分类为当前分类回填</div>
</body></html>"""
...
<div class="footer">生成: scripts/gen_market.py · 每格 = 当日板块内个股涨跌幅等权平均 · 申万分类为当前分类回填历史 · <a href="market_overview.html" style="color:var(--blue)">← 返回整体行情</a></div>
<script src="vendor/echarts.min.js"></script>
<script>window.echarts || document.write(`<script src='https://cdn.jsdelivr.net/npm/echarts@5.4.3/dist/echarts.min.js'><\\/script>`);</script>
<script>{js}</script>
</body></html>"""
...
def main():
    ...
    if not a.no_nav:
        try:
            ...
            generate()
            from scripts.gen_mobile import generate as generate_mobile
            generate_mobile()
