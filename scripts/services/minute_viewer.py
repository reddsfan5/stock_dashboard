...
.card{{background:var(--card);border:1px solid var(--border);border-radius:12px;box-shadow:0 3px 12px rgba(30,45,75,.05);display:flex;flex-direction:column}}.quote{{padding:16px 18px 8px;display:flex;align-items:flex-end;gap:16px;flex-wrap:wrap}}
...
#chart{{height:650px;width:100%;touch-action:pan-y;overscroll-behavior:contain}}#chart.is-scrubbing{{touch-action:none}}.status{{padding:0 18px 14px;color:var(--muted);font-size:12px;display:flex;justify-content:space-between;gap:10px}}.notice{{display:none;margin-bottom:12px;padding:10px 14px;border-radius:8px;background:#fff6dc;color:#765a00;font-size:13px}}
...
.page{{padding:6px 8px 78px}}
.topbar{{align-items:stretch;flex-direction:column;gap:6px;margin-bottom:6px}}
.brand{{display:none}}
.search-wrap{{max-width:none}}
.search-box{{border-radius:8px;box-shadow:none}}
#searchInput{{padding:8px 10px;font-size:13px}}
#searchBtn{{padding:0 12px;font-size:13px}}
.results{{top:40px;max-height:40vh}}
.toolbar{{justify-content:space-between;gap:6px}}
.toolbar button,.toolbar a,#dateSelect{{height:30px;padding:0 8px;font-size:12px;border-radius:7px}}
.card{{display:flex;flex-direction:column}}
.quote{{padding:8px 10px 4px;gap:6px 10px;align-items:center}}
.identity{{min-width:0;flex:1}}
.identity h1{{font-size:15px;margin:0 0 2px;line-height:1.2}}
.identity .sub{{font-size:11px;line-height:1.3}}
.last{{font-size:22px;line-height:1.1}}
.change{{font-size:13px}}
.stats{{grid-template-columns:repeat(3,1fr);gap:4px;min-width:100%}}
.stat{{border-left:0;padding:4px 6px;background:color-mix(in srgb,var(--muted) 14%,transparent);border-radius:6px}}
.stat .label{{font-size:10px;margin-bottom:1px}}
.stat .value{{font-size:12px}}
.quote{{order:1}}
#chart{{order:2;height:clamp(300px,52dvh,560px);min-height:280px}}
.intraday-replay{{order:3;padding:6px 8px;gap:5px}}
.status{{order:4;display:none}}
.notice{{margin-bottom:6px;padding:8px 10px;font-size:12px}}
}}
@media(max-width:760px) and (max-height:720px){{
#chart{{height:clamp(260px,48dvh,480px)}}
...
window.__minuteScrubActive=false;let minuteLongPressTimer=null,minuteTouchStart=null,minuteScrubMove=null;
function exitMinuteScrub(){{
  if(minuteLongPressTimer){{clearTimeout(minuteLongPressTimer);minuteLongPressTimer=null}}
  minuteTouchStart=null;
  const el=$('chart');
  if(minuteScrubMove&&el){{el.removeEventListener('touchmove',minuteScrubMove);minuteScrubMove=null}}
  if(!window.__minuteScrubActive){{if(el)el.classList.remove('is-scrubbing');return}}
  window.__minuteScrubActive=false;
  if(el)el.classList.remove('is-scrubbing');
  try{{chart.dispatchAction({{type:'hideTip'}})}}catch(e){{}}
  const coarse=(window.matchMedia('(pointer:coarse)').matches)||window.matchMedia('(max-width:760px)').matches;
  try{{chart.setOption({{tooltip:{{trigger:coarse?'none':'axis',triggerOn:coarse?'none':'mousemove|click',showContent:!coarse}},axisPointer:{{show:!coarse}}}},false)}}catch(e){{}}
}}
function updateMinuteScrub(touch){{
  if(!current||!current.points||!current.points.length)return;
  const el=$('chart'),rect=el.getBoundingClientRect();
  const x=touch.clientX-rect.left,y=touch.clientY-rect.top;
  let idx=-1;
  try{{const pt=chart.convertFromPixel({{gridIndex:0}},[x,y]);if(pt&&typeof pt[0]==='number')idx=Math.round(pt[0])}}catch(e){{}}
  if(idx<0){{try{{const pt2=chart.convertFromPixel({{xAxisIndex:0}},[x]);if(typeof pt2==='number')idx=Math.round(pt2);else if(pt2&&typeof pt2[0]==='number')idx=Math.round(pt2[0])}}catch(e){{}}}}
  if(idx<0||idx>=current.points.length)return;
  try{{chart.dispatchAction({{type:'showTip',seriesIndex:0,dataIndex:idx}})}}catch(e){{}}
}}
function enterMinuteScrub(touch){{
  if(window.__minuteScrubActive)return;
  window.__minuteScrubActive=true;
  const el=$('chart');
  if(el)el.classList.add('is-scrubbing');
  try{{navigator.vibrate&&navigator.vibrate(10)}}catch(e){{}}
  try{{chart.setOption({{tooltip:{{trigger:'axis',triggerOn:'none',show:true,showContent:true}},axisPointer:{{show:true,type:'line'}}}},false)}}catch(e){{}}
  if(el&&!minuteScrubMove){{
    minuteScrubMove=function(e){{if(!window.__minuteScrubActive)return;e.preventDefault();const t=e.touches&&e.touches[0];if(t)updateMinuteScrub(t)}};
    el.addEventListener('touchmove',minuteScrubMove,{{passive:false}});
  }}
  if(touch)updateMinuteScrub(touch);
}}
(function bindMinuteScrub(){{
  const el=$('chart');
  if(!el||el.dataset.scrubBound==='1')return;
  el.dataset.scrubBound='1';
  el.addEventListener('touchstart',function(e){{
    if(!e.touches||e.touches.length!==1)return;
    if(window.__minuteScrubActive)exitMinuteScrub();
    const t=e.touches[0];
    minuteTouchStart={{x:t.clientX,y:t.clientY}};
    if(minuteLongPressTimer)clearTimeout(minuteLongPressTimer);
    minuteLongPressTimer=setTimeout(function(){{minuteLongPressTimer=null;enterMinuteScrub(t)}},320);
  }},{{passive:true}});
  el.addEventListener('touchmove',function(e){{
    if(window.__minuteScrubActive||!minuteTouchStart||!e.touches||!e.touches[0])return;
    const t=e.touches[0],dx=t.clientX-minuteTouchStart.x,dy=t.clientY-minuteTouchStart.y;
    if(Math.hypot(dx,dy)>10){{if(minuteLongPressTimer){{clearTimeout(minuteLongPressTimer);minuteLongPressTimer=null}}}}
  }},{{passive:true}});
  const end=function(){{if(minuteLongPressTimer){{clearTimeout(minuteLongPressTimer);minuteLongPressTimer=null}}exitMinuteScrub()}};
...
function renderChart(data){{const pts=data.points,times=pts.map(p=>p.time),closes=pts.map(p=>p.close),vwaps=pts.map(p=>p.vwap),vols=pts.map(p=>p.volume),base=data.prev_close,extrema=pts.flatMap(p=>[p.high,p.low]);let dev=Math.max(...extrema.map(v=>Math.abs(v-base)),Math.abs(base)*.002)*1.08;const ymin=base-dev,ymax=base+dev,pct=Math.max(dev/base*100,.2);const narrow=window.matchMedia('(max-width:760px)').matches;const coarse=(window.matchMedia('(pointer:coarse)').matches)||narrow;const scrubbing=!!window.__minuteScrubActive;const gL=narrow?44:68,gR=narrow?48:72,gTop=narrow?22:35,gH=narrow?'66%':'62%',gTop2=narrow?'78%':'76%',gH2=narrow?'14%':'15%',dzH=narrow?14:18;chart.setOption({{animation:false,grid:[{{left:gL,right:gR,top:gTop,height:gH}},{{left:gL,right:gR,top:gTop2,height:gH2}}],axisPointer:{{link:[{{xAxisIndex:[0,1]}}],show:!coarse||scrubbing,label:{{backgroundColor:'#586174'}}}},tooltip:{{trigger:coarse&&!scrubbing?'none':'axis',triggerOn:coarse?'none':'mousemove|click',showContent:!coarse||scrubbing,axisPointer:{{type:'line',snap:true}},backgroundColor:'rgba(27,31,40,.94)',borderWidth:0,padding:[6,8],textStyle:{{color:'#fff',fontSize:12}},confine:true,position:function(pos,params,dom,rect,size){{const box=size.contentSize||[0,0],view=size.viewSize||[0,0],touch=window.matchMedia('(max-width:760px), (pointer:coarse)').matches;if(touch){{const x=Math.max(8,Math.min(pos[0]-box[0]/2,view[0]-box[0]-8));return[x,8]}}const x=pos[0]+16+box[0]>view[0]?pos[0]-box[0]-12:pos[0]+12;const y=pos[1]-box[1]-12<8?pos[1]+12:pos[1]-box[1]-12;return[x,y]}},formatter:params=>{{if(!params.length)return'';const i=params[0].dataIndex,p=pts[i],color=p.change_pct>=0?'#ff6b6b':'#41d49a';return`<div style="line-height:1.35;white-space:nowrap"><span style="opacity:.85">${{p.time}}</span>　<span style="color:${{color}};font-weight:700">${{price(p.close)}}</span>　<span style="color:${{color}}">${{signed(p.change_pct)}}</span><br><span style="opacity:.8;font-size:11px">量 ${{compact(p.volume)}}　均 ${{price(p.vwap)}}</span></div>`}}}},xAxis:[{{type:'category',gridIndex:0,data:times,boundaryGap:false,axisLabel:{{show:false}},axisTick:{{show:false}},axisLine:{{lineStyle:{{color:'#d9dde6'}}}}}},{{type:'category',gridIndex:1,data:times,boundaryGap:true,axisLabel:{{color:'#7b8190',formatter:(v,i)=>i%30===0?v:''}},axisTick:{{show:false}},axisLine:{{lineStyle:{{color:'#d9dde6'}}}}}}],yAxis:[{{type:'value',gridIndex:0,min:ymin,max:ymax,scale:true,splitNumber:6,axisLabel:{{formatter:v=>price(v),color:v=>v>=base?'#e5484d':'#16a36a'}},splitLine:{{lineStyle:{{color:'#edf0f5',type:'dashed'}}}}}},{{type:'value',gridIndex:0,min:-pct,max:pct,position:'right',splitNumber:6,axisLabel:{{formatter:v=>`${{v>=0?'+':''}}${{v.toFixed(2)}}%`,color:v=>v>=0?'#e5484d':'#16a36a'}},splitLine:{{show:false}}}},{{type:'value',gridIndex:1,scale:true,axisLabel:{{formatter:v=>compact(v),color:'#7b8190'}},splitLine:{{show:false}}}}],dataZoom:[{{type:'inside',xAxisIndex:[0,1],filterMode:'none',disabled:coarse}},{{type:'slider',xAxisIndex:[0,1],bottom:4,height:dzH,showDetail:false,borderColor:'#e5e8ef'}}],series:[{{name:'价格',type:'line',xAxisIndex:0,yAxisIndex:0,data:closes,showSymbol:false,lineStyle:{{width:1.4,color:'#3478f6'}},areaStyle:{{color:{{type:'linear',x:0,y:0,x2:0,y2:1,colorStops:[{{offset:0,color:'rgba(52,120,246,.18)'}},{{offset:1,color:'rgba(52,120,246,.01)'}}]}}}},markLine:{{silent:true,symbol:'none',label:{{formatter:`前收 ${{price(base)}}`,position:'insideEndTop',color:'#8a909e'}},lineStyle:{{color:'#9ca3af',type:'dashed'}},data:[{{yAxis:base}}]}}}},{{name:'均价',type:'line',xAxisIndex:0,yAxisIndex:0,data:vwaps,showSymbol:false,lineStyle:{{width:1,color:'#e5a000'}}}},{{name:'成交量',type:'bar',xAxisIndex:1,yAxisIndex:2,data:vols,itemStyle:{{color:p=>pts[p.dataIndex].direction>=0?'#e5484d':'#16a36a'}}}}]}},true)}}
