#!/usr/bin/env python3
"""策略10: MACD金叉+放量 — 经典指标组合。结果: -86% / 110笔 / 胜率50%"""
import argparse, os, sys, time; import numpy as np, pandas as pd; from tqdm import tqdm
PROJECT_DIR=os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))); sys.path.insert(0,PROJECT_DIR)
...
def run(capital=50000,start_date="2022-01-01",hold_days=20,top_n=5):
    ...
    for date in tqdm(dates,desc="MACD金叉"):
    ...
    print_stats(trades,cash,capital,start_date,str(ld)[:10],title="MACD金叉+放量",extra_info=f"DIF上穿DEA+量>5日均量1.2倍 | {hold_days}天×{top_n}只")
