#!/usr/bin/env python3
"""
策略4: 超卖均值回归 — 跌到20日低点附近买入，等反弹

结果: -7.2% / 305笔 / 胜率33.8% (最接近打平)
教训: 支撑位在A股经常被打穿，胜率不足导致微亏
"""
...
OUTPUT_HTML = os.path.join(PROJECT_DIR, "output", "strategy_04.html")

class MeanReversionStrategy(SimStrategy):
    def scan_signals(self, data, code_to_name, start_date, **kwargs):
        ...
        for code, grp in tqdm(cache.groupby("代码"), desc="扫描", unit="只"):
            ...
            for i in range(22, len(grp)):
                ...
                if pos > 3: continue
                if cl[i] <= op[i]: continue  # 收阴不买
                if i + 1 >= len(grp): continue
                all_signals.append({"代码": code, "名称": code_to_name.get(code,""), "触发日": dt[i+1], "区间位置%": round(pos,1)})
    ...
    def process_day(self, ctx: DayContext):
        ...
        for s in today[:3]:
            ...
            ctx.cash -= cost
            new.append(Position(code=s["代码"], shares=sh, buy_price=c, total_cost=cost, target_price=round(c*1.05,2), stop_price=round(c*0.97,2), buy_date=ctx.date, buy_day_low=l))
        return new


    def get_report_meta(self):
        return {"title": "📉 策略4：超卖均值回归", "subtitle": "跌到20日低点附近+收阳确认→博反弹 · +5%止盈 · -3%止损", "params_html": """💡 <b>选股：</b>收盘距20日最低<3% + 20日振幅10~35% + 当日收阳<br>💡 <b>买入：</b>次日收盘价，30%仓位，每天最多3只<br>💡 <b>卖出：</b>+5%止盈 / -3%止损<br>💡 <b>结果：</b>-7% ❌ — 支撑位在A股经常被打穿"""}
...
if __name__ == "__main__":
    ...
    SimEngine(MeanReversionStrategy()).run(capital=a.capital, start_date=a.start, target_pct=5.0, commission_rate=0.0001, output_html=OUTPUT_HTML)
