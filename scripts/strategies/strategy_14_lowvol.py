...
def run(capital=50_000, start_date="2022-01-01", hold_days=20, top_n=5,
    ...
    def selector(ctx):
        ...
        mask = (
            strong & near_high & volume_ok & low_amplitude
            & score.notna() & (score > -5)
        )
    ...
    return run_rebalance_report(
        ...
        title="策略14：低波动强趋势",
        subtitle=f"MA150>MA200 + 20日振幅<15% · {hold_days}日轮动{top_n}只",
        params_html=(
            "💡 <b>趋势：</b>收盘>MA150>MA200，且接近52周高点<br>"
            "💡 <b>低波：</b>近20日振幅<15%，排除急涨急跌标的<br>"
            f"💡 <b>排序：</b>近20日动量，等权{top_n}只，持有{hold_days}个交易日<br>"
            "💡 <b>前视修复：</b>指标使用前一交易日，下一交易日收盘成交"
        ),
        stats_title="低波动强趋势选股",
        stats_extra=f"MA150>MA200+近高点+低振幅 | {hold_days}日×{top_n}只",
