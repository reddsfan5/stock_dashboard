...
本次响应式界面改版、Wi-Fi 访问和验收结果见 [22-界面优化与局域网访问](22-界面优化与局域网访问.md)。
公司等外网用手机访问交互 Web，见 [26-Tailscale外网访问](26-Tailscale外网访问.md)。
跨页面大改动的动机、思路与取舍，见 [28-大改动设计日志](28-大改动设计日志.md)。
...
| [33-节假日效应回测](33-节假日效应回测.md) | 元旦/春节/五一/中秋/国庆休市前后表现、风险偏好、交互页面与当年位置 | 策略研究者 |
...
| [28-大改动设计日志](28-大改动设计日志.md) | 大改动动机、思路、取舍与效果（持续追加） | 架构与界面维护者 |
| [29-每日精选](29-每日短名单理由卡.md) | 多维拼装的每日 10～15 只精选标的与后续收益监控 | 选股使用者 |
| [30-板块相关点云](30-板块相关点云.md) | 点云节点、同步相关、领先传导的颜色与连线含义 | 板块研究与复盘使用者 |
| [31-观察池收益监控](31-观察池收益监控.md) | 按实际加入批次观察池后续峰值、收盘与超额收益 | 观察池复盘使用者 |
...
python -m scripts.research.holiday_effect          # 节假日效应（含页面）
...
- [板块图谱内容维护](23-板块图谱内容维护.md)
- [板块归属与主营构成数据库](24-板块归属与主营构成数据库.md)

- [27-多用户账户](27-多用户账户.md) — 登录与个人数据隔离
